...
def gerente(): # define qual thread pode fazer operações sobre sobre cada conta, mexendo na segunda linha do array 'gerencia'
    ...
    global ThreasList
    while True:
        if not threads_are_working(ThreadsList): return # se as threads que fazem requsições ja tiverem terminado esta fecha.
        ...
        sleep(2) # para a simuação não ficar muito rápida
...
def ThreadOperador(ThreadID, NuBank): # thread que opera sobre as contas,fazendo requisições de operação
    ...
    while True :
        nops += 1
        if nops == 3: break # para parar apos algumas operações realizadas
        ...
        sleep(1) # dorme para a 'simulação nao ser tão rapida
